<p align="center">
 <img src="Images/OitiHeader.png"/>
</p>

# Changelog

### [Versão 9.5.8](https://github.com/oititec/android-oiti-versions/releases/tag/9.5.8)

Data: 02/06/2026

- Atualização da FaceTec [9.7.126]
- Atualização dos fluxos internos para melhorar tratativas de erro.

### [Versão 9.5.6](https://github.com/oititec/android-oiti-versions/releases/tag/9.5.6)

Data: 10/03/2026

- Atualização da FaceTec [9.7.115]

### [Versão 9.5.5](https://github.com/oititec/android-oiti-versions/releases/tag/9.5.5)

Data: 18/02/2026

- Ajustes nos _callbacks_ do Liveness.

### [Versão 9.5.4](https://github.com/oititec/android-oiti-versions/releases/tag/9.5.4)

Data: 27/01/2026

- Atualização da FaceTec [9.7.109]
  - Melhorias na acurácia e compatibilidade do 3D Liveness (animações e transições mais suaves em diversos dispositivos).
  - APIs novas pra customizar tempo de exibição das animações no fluxo de Liveness (display time configurável entre 1,5 s e 3 s).
  - Remoção das APIs de timeout de sessão de Liveness não operacionais, deixando o SDK mais limpo.
  - Segurança de Liveness reforçada com testes de injeção de vídeo Nível 4 e 5 pra mitigar ameaças de IA Generativa.iva.

### [Versão 9.5.3](https://github.com/oititec/android-oiti-versions/releases/tag/9.5.3)

Data: 18/12/2025

- Atualização da FaceTec [9.7.102]
  - Melhorias na acurácia e compatibilidade do 3D Liveness (animações e transições mais suaves em diversos dispositivos).
  - APIs novas pra customizar tempo de exibição das animações no fluxo de Liveness (display time configurável entre 1,5 s e 3 s).
  - Remoção das APIs de timeout de sessão de Liveness não operacionais, deixando o SDK mais limpo.
  - Segurança de Liveness reforçada com testes de injeção de vídeo Nível 4 e 5 pra mitigar ameaças de IA Generativa.

### [Versão 9.5.2](https://github.com/oititec/android-oiti-versions/releases/tag/9.5.2)

Data: 17/11/2025

- Atualização da FaceTec [9.7.96]
  - Melhorias na acurácia e compatibilidade do 3D Liveness (animações e transições mais suaves em diversos dispositivos).
  - APIs novas pra customizar tempo de exibição das animações no fluxo de Liveness (display time configurável entre 1,5 s e 3 s).
  - Remoção das APIs de timeout de sessão de Liveness não operacionais, deixando o SDK mais limpo.
  - Segurança de Liveness reforçada com testes de injeção de vídeo Nível 4 e 5 pra mitigar ameaças de IA Generativa.

### [Versão 9.5.1](https://github.com/oititec/android-oiti-versions/releases/tag/9.5.1)

Data: 02/10/2025

- Atualização da FaceTec [9.7.87]
  - Melhorias na acurácia e compatibilidade do 3D Liveness (animações e transições mais suaves em diversos dispositivos).
  - APIs novas pra customizar tempo de exibição das animações no fluxo de Liveness (display time configurável entre 1,5 s e 3 s).
  - Remoção das APIs de timeout de sessão de Liveness não operacionais, deixando o SDK mais limpo.
  - Segurança de Liveness reforçada com testes de injeção de vídeo Nível 4 e 5 pra mitigar ameaças de IA Generativa.

### [Versão 9.5.0](https://github.com/oititec/android-oiti-versions/releases/tag/9.5.0)

Data: 16/09/2025

### Conformidade Google Play

- SDK atualizado para suportar dispositivos com **page size de 16 KB**.  
  A partir de **novembro/2025**, builds sem compatibilidade poderão ser rejeitados pela Google Play.

### Atualizações

- Atualização de bibliotecas legadas e alinhamento do toolchain.

### ⚠️ Breaking Changes

- Requer **Java 17** (`sourceCompatibility`/`targetCompatibility` = 17, `jvmTarget` = 17).
- Atualizado para **Kotlin 2.1.21** e **Coroutines 1.10.x**.
- **Retrofit 3.x + OkHttp 5.x**.
- **BouncyCastle** atualizado para `bcprov-jdk15to18:1.81`
  > Será necessário alinhar a versão em caso de conflito.
- **AndroidX** atualizada (Core 1.17, AppCompat 1.7, Fragment 1.8, Material 1.13).
  > Pode exigir `compileSdk` mais recente.
- **Koin 4.1.1**

### [Versão 9.4.10](https://github.com/oititec/android-oiti-versions/releases/tag/9.4.10)

Data: 28/08/2025

- Atualização da FaceTec [9.7.81]
  - Melhorias na acurácia e compatibilidade do 3D Liveness (animações e transições mais suaves em diversos dispositivos).
  - APIs novas pra customizar tempo de exibição das animações no fluxo de Liveness (display time configurável entre 1,5 s e 3 s).
  - Remoção das APIs de timeout de sessão de Liveness não operacionais, deixando o SDK mais limpo.
  - Segurança de Liveness reforçada com testes de injeção de vídeo Nível 4 e 5 pra mitigar ameaças de IA Generativa.

### [Versão 9.4.9](https://github.com/oititec/android-oiti-versions/releases/tag/9.4.9)

Data: 14/08/2025

- Atualização da FaceTec [9.7.79]
  - Melhorias na acurácia e compatibilidade do 3D Liveness (animações e transições mais suaves em diversos dispositivos).
  - APIs novas pra customizar tempo de exibição das animações no fluxo de Liveness (display time configurável entre 1,5 s e 3 s).
  - Remoção das APIs de timeout de sessão de Liveness não operacionais, deixando o SDK mais limpo.
  - Segurança de Liveness reforçada com testes de injeção de vídeo Nível 4 e 5 pra mitigar ameaças de IA Generativa.

### [Versão 9.4.8](https://github.com/oititec/android-oiti-versions/releases/tag/9.4.8)

Data: 14/07/2025

- Atualização da FaceTec [9.7.75]
  - Melhorias na acurácia e compatibilidade do 3D Liveness (animações e transições mais suaves em diversos dispositivos).
  - APIs novas pra customizar tempo de exibição das animações no fluxo de Liveness (display time configurável entre 1,5 s e 3 s).
  - Remoção das APIs de timeout de sessão de Liveness não operacionais, deixando o SDK mais limpo.
  - Segurança de Liveness reforçada com testes de injeção de vídeo Nível 4 e 5 pra mitigar ameaças de IA Generativa.

### [Versão 9.4.7](https://github.com/oititec/android-oiti-versions/releases/tag/9.4.7)

Data: 23/05/2025

- Atualização da FaceTec [9.7.68]
  - Melhorias na acurácia e compatibilidade do 3D Liveness (animações e transições mais suaves em diversos dispositivos).
  - APIs novas pra customizar tempo de exibição das animações no fluxo de Liveness (display time configurável entre 1,5 s e 3 s).
  - Remoção das APIs de timeout de sessão de Liveness não operacionais, deixando o SDK mais limpo.
  - Segurança de Liveness reforçada com testes de injeção de vídeo Nível 4 e 5 pra mitigar ameaças de IA Generativa.

### [Versão 9.4.6](https://github.com/oititec/android-oiti-versions/releases/tag/9.4.6)

Data: 15/05/2025

- Atualização da Facetec [9.7.66]
  - Melhorias na Taxa de Sucesso e Compatibilidade com Dispositivos.
  - Melhorias de Desempenho/Velocidade.
  - Melhorias na Estabilidade e no Gerenciamento de Memória.
  - Melhorias nas Verificações de Injeção de Vídeo no lado do Dispositivo e no Servidor para mitigar Ameaças de IA Generativa.

### [Versão 9.4.5](https://github.com/oititec/android-oiti-versions/releases/tag/9.4.5)

Data: 24/04/2025

- Atualização da Facetec [9.7.63]
  - Melhorias na Taxa de Sucesso e Compatibilidade com Dispositivos.
  - Melhorias de Desempenho/Velocidade.
  - Melhorias na Estabilidade e no Gerenciamento de Memória.
  - Melhorias nas Verificações de Injeção de Vídeo no lado do Dispositivo e no Servidor para mitigar Ameaças de IA Generativa.
- Adicionado novo parâmetro `vocalGuidanceCustomization` no `Liveness3DTheme` que recebe um enum de `Liveness3DVoiceOption`, com isso é possivel habilitar o voice da facetec para feedback sonoro do desafio de liveness. Por padrão essa propriedade vem `NO_VOCAL_GUIDANCE`.

### [Versão 9.4.4](https://github.com/oititec/android-oiti-versions/releases/tag/9.4.4)

Data: 04/04/2025

- Atualização da Facetec [9.7.61]
  - Melhorias na Taxa de Sucesso e Compatibilidade com Dispositivos.
  - Melhorias de Desempenho/Velocidade.
  - Melhorias na Estabilidade e no Gerenciamento de Memória.
  - Melhorias nas Verificações de Injeção de Vídeo no lado do Dispositivo e no Servidor para mitigar Ameaças de IA Generativa.
- Correção de uma falha relacionada a rede durante o processo de liveness.

### [Versão 9.4.3](https://github.com/oititec/android-oiti-versions/releases/tag/9.4.3)

Data: 06/03/2025

- Atualização da Facetec [9.7.55]
  - Melhorias na Taxa de Sucesso e Compatibilidade com Dispositivos.
  - Melhorias de Desempenho/Velocidade.
  - Melhorias na Estabilidade e no Gerenciamento de Memória.
  - Melhorias nas Verificações de Injeção de Vídeo no lado do Dispositivo e no Servidor para mitigar Ameaças de IA Generativa.

### [Versão 9.4.2](https://github.com/oititec/android-oiti-versions/releases/tag/9.4.2)

Data: 29/01/2025

- Atualização da Facetec [9.7.50]
  - Melhorias na Taxa de Sucesso e Compatibilidade com Dispositivos.
  - Melhorias de Desempenho/Velocidade.
  - Melhorias na Estabilidade e no Gerenciamento de Memória.
  - Melhorias nas Verificações de Injeção de Vídeo no lado do Dispositivo e no Servidor para mitigar Ameaças de IA Generativa.

### [Versão 9.4.1](https://github.com/oititec/android-oiti-versions/releases/tag/9.4.1)

Data: 17/12/2024

- Atualização da Facetec - versão 9.7.45
  - Melhorias na Taxa de Sucesso e Compatibilidade com Dispositivos.
  - Melhorias de Desempenho/Velocidade.
  - Melhorias na Estabilidade e no Gerenciamento de Memória.
  - Melhorias nas Verificações de Injeção de Vídeo no lado do Dispositivo e no Servidor para mitigar Ameaças de IA Generativa.
- Aprimoramento da coleta de Logs para casos de quebra do SDK.

### [Versão 9.4.0](https://github.com/oititec/android-oiti-versions/releases/tag/9.4.0)

Data: 30/10/2024

- Atualização da Facetec - versão 9.7.37
  - Melhorias na Taxa de Sucesso e Compatibilidade com Dispositivos.
  - Melhorias de Desempenho/Velocidade.
  - Melhorias na Estabilidade e no Gerenciamento de Memória.
  - Melhorias nas Verificações de Injeção de Vídeo no lado do Dispositivo e no Servidor para mitigar Ameaças de IA Generativa.
- Aprimoramento da coleta de Logs para casos de quebra do SDK.

### [Versão 9.3.4](https://github.com/oititec/android-oiti-versions/releases/tag/9.3.4)

Data: 09/10/2024

- Correção no método criado na versão anterior [9.3.3](https://github.com/oititec/android-oiti-versions/releases/tag/9.3.3) ao qual ocorria um erro ao utilizar dados móveis.

### [Versão 9.3.3](https://github.com/oititec/android-oiti-versions/releases/tag/9.3.3)

Data: 08/10/2024

- Correção para um bug que ocorria ao utilizar redes moveis e executar o SDK.

### [Versão 9.3.2](https://github.com/oititec/android-oiti-versions/releases/tag/9.3.2)

Data: 24/09/2024

- Atualização da Facetec - versão 9.7.32
  - Não há mudanças que causem quebras significativas, exceto a remoção de algumas APIs menores que foram depreciadas.
    As atualizações da v9.6.x para a v9.7.x são realizadas exatamente como as atualizações da v9.6.x.
    Consulte o Guia de Atualização da v9.7 para mais informações.
  - Melhorias na Taxa de Sucesso e Compatibilidade com Dispositivos.
  - Melhorias de Desempenho/Velocidade.
  - Melhorias na Estabilidade e no Gerenciamento de Memória.
  - Melhorias nas Verificações de Injeção de Vídeo no lado do Dispositivo e no Servidor para mitigar Ameaças de IA Generativa.

### [Versão 9.3.1](https://github.com/oititec/android-oiti-versions/releases/tag/9.3.1)

Data: 21/07/2024

- Dicionado observabilidade no Liveness 3D

### [Versão 9.3.0](https://github.com/oititec/android-oiti-versions/releases/tag/9.3.0)

Data: 16/07/2024

- Atualização da Facetec - versão 9.6.101
  - Feedback genérico atualizado.
  - Numerosas correções de polimento de UI/UX.
  - Registro aprimorado em casos de erro.
  - Registro aprimorado para determinados casos de erro.
  - Pequenas melhorias nos modos de pouca luz e escurecimento automático.
  - Numerosas correções de compatibilidade, estabilidade e desempenho.
  - Corrigidas várias exceções de casos extremos que fazem com que UNKNOWN_INTERNAL_ERROR seja retornado.
  - Adicionadas novas verificações de injeção de vídeo no lado do dispositivo e no servidor para mitigar ameaças de IA generativa.
  - Melhorias de desempenho em dispositivos de nível inferior e intermediário, levando a melhorias na taxa de sucesso.
- Atualização na arquitetura
- Atualização no Gradle version para 8.7
- Atualização no Android Gradle plugin version para 8.5.0

### [Versão 9.2.9](https://github.com/oititec/android-oiti-versions/releases/tag/9.2.9)

Data: 18/06/2024

- Atualização da Facetec - versão 9.6.96
  - Feedback genérico atualizado.
  - Numerosas correções de polimento de UI/UX.
  - Registro aprimorado em casos de erro.
  - Registro aprimorado para determinados casos de erro.
  - Pequenas melhorias nos modos de pouca luz e escurecimento automático.
  - Numerosas correções de compatibilidade, estabilidade e desempenho.
  - Corrigidas várias exceções de casos extremos que fazem com que UNKNOWN_INTERNAL_ERROR seja retornado.
  - Adicionadas novas verificações de injeção de vídeo no lado do dispositivo e no servidor para mitigar ameaças de IA generativa.
  - Melhorias de desempenho em dispositivos de nível inferior e intermediário, levando a melhorias na taxa de sucesso.

### [Versão 9.2.8](https://github.com/oititec/android-oiti-versions/releases/tag/9.2.8)

Data: 22/05/2024

- Atualização da Facetec - versão 9.6.90
  - Feedback genérico atualizado.
  - Numerosas correções de polimento de UI/UX.
  - Registro aprimorado em casos de erro.
  - Registro aprimorado para determinados casos de erro.
  - Pequenas melhorias nos modos de pouca luz e escurecimento automático.
  - Numerosas correções de compatibilidade, estabilidade e desempenho.
  - Corrigidas várias exceções de casos extremos que fazem com que UNKNOWN_INTERNAL_ERROR seja retornado.
  - Adicionadas novas verificações de injeção de vídeo no lado do dispositivo e no servidor para mitigar ameaças de IA generativa.
  - Melhorias de desempenho em dispositivos de nível inferior e intermediário, levando a melhorias na taxa de sucesso.

### [Versão 9.2.7](https://github.com/oititec/android-oiti-versions/releases/tag/9.2.7)

Data: 22/04/2024

- Atualização dos retornos:
  - corrigido cenário onde `TRANSACTION_NOT_COMPLETED` era retornado de forma incorreta.

### [Versão 9.2.6](https://github.com/oititec/android-oiti-versions/releases/tag/9.2.6)

Data: 28/03/2024

- Atualização da FaceTec - versão 9.6.80:
  - Feedback genérico atualizado.
  - Numerosas correções de polimento de UI/UX.
  - Registro aprimorado em casos de erro.
  - Registro aprimorado para determinados casos de erro.
  - Pequenas melhorias nos modos de pouca luz e escurecimento automático.
  - Numerosas correções de compatibilidade, estabilidade e desempenho.
  - Adicionadas novas verificações de injeção de vídeo no lado do dispositivo e no servidor para mitigar ameaças de IA generativa.
  - Melhorias de desempenho em dispositivos de nível inferior e intermediário, levando a melhorias na taxa de sucesso.
- Aprimoramento para registrar logs de falhas e crashs

### [Versão 9.2.5](https://github.com/oititec/android-oiti-versions/releases/tag/9.2.5)

Data: 21/03/2024

- Aprimoramento de erros genéricos
- Novo retorno implementado para casos onde a appkey não é inicializada corretamente `LIVENESS_NOT_INITIALIZED`

### [Versão 9.2.4](https://github.com/oititec/android-oiti-versions/releases/tag/9.2.4)

Data: 05/03/2024

- Foi Implementada uma nova validação para a appkey
- Alteração de retornos para um melhor entendimento. O retorno agora passa a ser `TRANSACTION_NOT_COMPLETED` para casos onde era retornado `NO_INTERNET_CONNECTION`.
- Correção onde em alguns casos apontando para o environment de PRD era encaminhado para HML

### [Versão 9.2.3](https://github.com/oititec/android-oiti-versions/releases/tag/9.2.3)

Data: 23/02/2024

- Correção Environment apontando para HML em casos onde era definido PRD.
- Correção `KoinJavaComponent.inject<Context>(Context::class.java)` gerando crashs no SDK em alguns cenários.

### [Versão 9.2.2](https://github.com/oititec/android-oiti-versions/releases/tag/9.2.2)

Data: 06/02/2024

- Atualização da FaceTec - versão 9.6.71
  - Feedback genérico atualizado.
  - Numerosas correções de polimento de UI/UX.
  - Registro aprimorado em casos de erro.
  - Registro aprimorado para determinados casos de erro.
  - Pequenas melhorias nos modos de pouca luz e escurecimento automático.
  - Numerosas correções de compatibilidade, estabilidade e desempenho.
  - Corrigidas várias exceções de casos extremos que fazem com que UNKNOWN_INTERNAL_ERROR seja retornado.
  - Adicionadas novas verificações de injeção de vídeo no lado do dispositivo e no servidor para mitigar ameaças de IA generativa.
  - Melhorias de desempenho em dispositivos de nível inferior e intermediário, levando a melhorias na taxa de sucesso.

### [Versão 9.2.1](https://github.com/oititec/android-oiti-versions/releases/tag/9.2.1)

Data: 22/01/2024

- Revisão no tagueamento do liveness 3D, validação dos eventos mapeados.
- Remoção da fonte Alvar essential.

### [Versão 9.2.0](https://github.com/oititec/android-oiti-versions/releases/tag/9.2.0)

Data: 29/12/2023

- Adicionado Device Intelligence
- Captura de Localização do Usuário
- Captura de Dados do Dispositivo do Usuário
- Captura Eventos do Usuário

**Necessário incluir a PERMISSÃO de LOCALIZAÇÃO:**
Seguir as instruções de permissões disponíveis [neste link.](https://devcenter.certiface.io/docs/guia-de-instalacao-android#passo-3-permiss%C3%A3o-de-localiza%C3%A7%C3%A3o)

### [Versão 9.1.0](https://github.com/oititec/android-oiti-versions/releases/tag/9.1.0)

Data: 05/12/2023

- Atualização da FaceTec - versão 9.6.63
- Novo laboratório testado e segurança confirmada, injeção de vídeo de nível 4 e 5 e segurança de desvio de câmera.

### [Versão 9.0.1](https://github.com/oititec/android-oiti-versions/releases/tag/9.0.1)

Data: 01/11/2023

- Atualização da FaceTec - versão 9.6.54
  - Feedback genérico atualizado.
  - Numerosas correções de polimento de UI/UX.
  - Registro aprimorado em casos de erro.
  - Pequenas melhorias nos modos de pouca luz e escurecimento automático.
  - Numerosas correções de compatibilidade, estabilidade e desempenho.
  - Adicionadas novas verificações de injeção de vídeo no lado do dispositivo e no servidor para mitigar ameaças de IA generativa.
  - Melhorias de desempenho em dispositivos de nível inferior e intermediário, levando a melhorias na taxa de sucesso.

### [Versão 9.0.0](https://github.com/oititec/android-oiti-versions/releases/tag/9.0.0)

Data: 04/10/2023

- Atualização do `PARAM_FONTS`
- Remoção do `PARAM_FONTS_NEW_API`
- Breaking changes e guia de migração para a [9.0.0](https://github.com/oititec/android-oiti-versions/blob/master/Liveness3D/Documentation/BreakingChange/Migration-Guide-9.0.0.md)

### [Versão 8.1.1](https://github.com/oititec/android-oiti-versions/releases/tag/8.1.1)

Data: 21/09/2023

- Atualização da FaceTec - versão 9.6.49
  - Adicionadas novas verificações de injeção de vídeo no lado do dispositivo e no servidor para mitigar ameaças de IA generativa.
  - Melhorias de desempenho em dispositivos de nível inferior e intermediário, levando a melhorias na taxa de sucesso.
  - Correções para melhorar a compatibilidade de vários dispositivos Samsung, Redmi e Xiaomi.
  - Numerosas correções de melhoria de estabilidade.
  - Mensagens de registro aprimoradas.

### [Versão 8.1.0](https://github.com/oititec/android-oiti-versions/releases/tag/8.1.0)

Data: 29/08/2023

- Nova forma para customizar as cores adicionada, permitindo a passagem de um hexadecimal String para o builder do Liveness3DTheme.

```kotlin
val liveness3DTheme = Liveness3DTheme.Builder()
                // Colors for hybrid
                .guidanceCustomizationBackgroundColors("#73997d")
                .feedbackCustomizationTextColor("#15d100")
                .build()
```

### [Versão 8.0.1](https://github.com/oititec/android-oiti-versions/releases/tag/8.0.1)

Data: 24/08/2023

- Atualização da FaceTec - versão 9.6.42
  - Adicionadas novas verificações de injeção de vídeo no lado do dispositivo e no servidor para mitigar ameaças de IA generativa.
  - Melhorias de desempenho em dispositivos de nível inferior e intermediário, levando a melhorias na taxa de sucesso.
  - Correções para melhorar a compatibilidade de vários dispositivos Samsung, Redmi e Xiaomi.
  - Numerosas correções de melhoria de estabilidade.

### [Versão 8.0.0](https://github.com/oititec/android-oiti-versions/releases/tag/8.0.0)

Data: 11/08/2023

- **Breaking changes**
  - Novo PARAM_FONTS adicionado para APIs abaixo do (API level 26)
  - Novo Param_FONTS_NEW_API adicionado
  - A estrutura do `Liveness3DTheme` teve algumas propriedades removidas, segue a lista:
    - `guidanceCustomizationHeaderFont`
    - `guidanceCustomizationSubtextFont`
    - `guidanceCustomizationButtonFont`
    - `guidanceCustomizationReadyScreenHeaderFont`
    - `guidanceCustomizationReadyScreenSubtextFont`
    - `guidanceCustomizationRetryScreenHeaderFont`
    - `guidanceCustomizationRetryScreenSubtextFont`
    - `resultScreenCustomizationMessageFont`
    - `feedbackCustomizationTextFont`
  - A customização das fontes agora é feita somente pelo dicionário de chave e referencia ou caminho da fonte, assim como é aplicado com os textos. `Liveness3DFontsKey`.
  - Breaking changes e guia de migração para a [8.0.0](https://github.com/oititec/android-oiti-versions/blob/master/Liveness3D/Documentation/BreakingChange/Migration-Guide-8.0.0.md)

### [Versão 7.1.2](https://github.com/oititec/android-oiti-versions/releases/tag/7.1.2)

Data: 21/07/2023

- Corrigido crash que ocorria com o custom loading nas plataformas hibridas.

### [Versão 7.1.1](https://github.com/oititec/android-oiti-versions/releases/tag/7.1.1)

Data: 17/07/2023

- Ajuste na propriedade RESULT_SUCCESS_MESSAGE á qual não aplicava customização.
- Revisão na estrutura da customização dos textos.

### [Versão 7.1.0](https://github.com/oititec/android-oiti-versions/releases/tag/7.1.0)

Data: 07/07/2023

- Atualização da FaceTec - versão 9.6.36
  - A propriedade feedbackEyesStraightAhead foi descontinuada.
  - Adicionada nova checagem de injeção de video no dispositivo e servidor para mitigar a ameaça das IAs Generativas.
  - Melhorias de estabilide.

### [Versão 7.0.0](https://github.com/oititec/android-oiti-versions/releases/tag/7.0.0)

Data: 21/06/2023

- Implementado custom loadings:
- Ponto de melhoria para tecnologias hibridas.

### [Versão 6.9.2](https://github.com/oititec/android-oiti-versions/releases/tag/6.9.2)

Data: 20/06/2023

- Ponto de correção encontrado com Koin:
- Ajuste na configuração do Koin para garantir inicialização e independência em apps que já possuem Koin ou não utilizam o framework.

### [Versão 6.9.1](https://github.com/oititec/android-oiti-versions/releases/tag/6.9.1)

Data: 15/06/2023

- Ponto de correção encontrado com Koin:
- Encontrado ponto onde dentro do application, android:name conflitava com o .App do manifest do SDK e não permitia o build do app.

### [Versão 6.9](https://github.com/oititec/android-oiti-versions/releases/tag/6.9)

Data: 30/05/2023

- Atualização do SDK para 6.9 trazendo melhorias de arquitetura interna
  - adicionado a possibilidade de customizar o campo resultScreenOverrideSuccessMessage que altera a mensagem de sucesso.
  - Atualizado modulo de uso interno para nova arquitetura
  - Várias correções de melhoria de estabilidade.

### [Versão 6.8](https://github.com/oititec/android-oiti-versions/releases/tag/6.8)

Data: 26/05/2023

- Atualização para Facetec v9.6.30
  - Adicionadas novas verificações de injeção de vídeo no lado do dispositivo e no lado do servidor para mitigar ameaças de IA generativas.
  - Melhorias de desempenho em dispositivos de nível baixo e intermediário, levando a melhorias na taxa de sucesso.
  - Correções para melhorar a compatibilidade de vários dispositivos Samsung e Redmi.
  - Várias correções de melhoria de estabilidade.

### 6.7

Data: 25/04/2023

- Atualizações de segurança / Facetec 9.6.25 / Melhorias de UX

### 6.6

Data: 04/04/2023

- Atualizações de segurança / Facetec 9.6.23

### 6.6

Data: 04/04/2023

- Correções de inicialização da Classe Liveness3DTextKey na Activity

### 6.5

- Correções de inicialização da Classe Liveness3DTextKey na Activity

### 6.4

- Melhorias de UX
- Correções de compatiblidade na importação da Classe Liveness3DTextKey

### 6.3

- Permitir customização das strings via objetos

### 6.2

Data: 13/03/2023

- Atualizações de segurança / Facetec 9.6.15

### 6.1

- Permitir customização da propriedade guidanceCustomizationReadyScreenHeaderFont

### 6.0

Data: 23/02/2023

##### Remoção das seguintes propriedades abaixo:

- guidanceCustomizationReadyScreenHeaderAttributedString
- guidanceCustomizationReadyScreenSubtextAttributedString
- guidanceCustomizationRetryScreenHeaderAttributedString
- guidanceCustomizationRetryScreenSubtextAttributedString

#### Novo formato para importar as dependências

- A partir da versão 6.0 somente é necessário declarar e importar o artefato do liveness 3D, não sendo mais necessário importar a lib da facetec paralelamente.

### 5.8.0

- Adicionado prevenção para clique duplo no botão de iniciar o liveness
- Permitir customização de botão habilitado/desabilitado
- Melhorias de UX

### 5.7.0

- Atualização da versão do Facetec SDK para 9.4.23
- Melhorias de UX
